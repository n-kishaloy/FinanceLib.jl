...
* bd  = CouponBond
"""
priceCouponBond(r::Float64, bd) = (bd.c/bd.freq) * sum(1.0 ./ (1+r).^((1:(bd.freq*bd.T)) ./ bd.freq )) + 1/(1+r)^bd.T
...
* bd  = CouponBond
"""
priceCouponBond(r::Rt.RateCurve, bd) = (bd.c/bd.freq) * sum( (i -> 1/(1+ Rt.rate(r,i/bd.freq))^(i/bd.freq) ).(1:(bd.freq*bd.T)) ) + 1/(1 + Rt.rate(r, bd.T))^bd.T
...
function ytmCouponBond(bd::CouponBond, P)
  cf = (i -> (i/bd.freq, bd.c/bd.freq)).(1:(bd.freq*bd.T))
